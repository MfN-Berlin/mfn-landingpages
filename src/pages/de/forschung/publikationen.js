...
const formatAPA = (pub, searchTerm) => {
  try {
    const authors = highlightMatches(formatAuthors(pub.mfn_authors || '', pub.unknown_authors || ''), searchTerm)
    const year = pub.year || 'n.d.'
    const title = pub.title 
      ? `<span class="font-bold">${highlightMatches(pub.title, searchTerm)}</span>` 
      : '<span class="font-bold">Untitled</span>'
    const journal = highlightMatches(pub.journal || 'Unknown Journal', searchTerm)
    const volume = pub.volume ? `<i>${highlightMatches(pub.volume, searchTerm)}</i>` : ''
    const issue = pub.issue ? `(${highlightMatches(pub.issue, searchTerm)})` : ''
    const pageInfo = pub.page_info 
      ? (typeof pub.page_info === 'string' && pub.page_info.startsWith('Article') 
        ...
        : `p. ${highlightMatches(pub.page_info, searchTerm)}`)
      : ''
    const doi = pub.upload_id && typeof pub.upload_id === 'string' && pub.upload_id.startsWith('10.') 
      ? `<a href="https://doi.org/${pub.upload_id}" target="_blank" rel="noopener noreferrer" class="underline text-Green-500 hover:text-Green-600">https://doi.org/${pub.upload_id}</a>` 
      : ''

    return `${authors}. (${year}). ${title}. <i>${journal}</i>, ${volume}${issue}, ${pageInfo}. ${doi}`.trim().replace(/\s+/g, ' ')
  } catch (error) {
